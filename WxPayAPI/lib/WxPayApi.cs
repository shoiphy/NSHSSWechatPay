...
namespace WxPayAPI
{
    public class WxPayApi
    {
        ...
        * 
        * Unified Order
        * @param WxPaydata inputObj input parameters to unified order
        * @param int timeOut overtime time
        * @throws WxPayException
        * @return Return on success, other throw exceptions
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            {
                throw new WxPayException("缺少统一支付接口必填参数out_trade_no！");//Missing out_trade_no
            }
            else if (!inputObj.IsSet("body"))
            {
                throw new WxPayException("缺少统一支付接口必填参数body！");//Missing body
            }
            else if (!inputObj.IsSet("total_fee"))
            {
                throw new WxPayException("缺少统一支付接口必填参数total_fee！");//Missing total_fee
            }
            else if (!inputObj.IsSet("trade_type"))
            {
                throw new WxPayException("缺少统一支付接口必填参数trade_type！");//Missing trade_type
            }

            //Associated parameter
            if (inputObj.GetValue("trade_type").ToString() == "JSAPI" && !inputObj.IsSet("openid"))
            {
                throw new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");//Missing openid,when trade_type is JSAPI
            }
            if (inputObj.GetValue("trade_type").ToString() == "NATIVE" && !inputObj.IsSet("product_id"))
            {
                throw new WxPayException("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！");//Missing product_id,when trade_type is JSAPI
            }

            //Asynchronous notification url is not set, then use the url in the configuration file
            if (!inputObj.IsSet("notify_url"))
            {
                inputObj.SetValue("notify_url", WxPayConfig.GetConfig().GetNotifyUrl());//Asynchronous notification url
            }

            //inputObj.SetValue("spbill_create_ip", WxPayConfig.GetConfig().GetIp());//client ip
            //If the terminal IP is not set, the terminal IP in the configuration file is used.
            if (!inputObj.IsSet("spbill_create_ip"))
            {
                inputObj.SetValue("spbill_create_ip", WxPayConfig.GetConfig().GetIp());//client IP
            }
            inputObj.SetValue("appid", WxPayConfig.GetConfig().GetAppID());//official account ID
            inputObj.SetValue("mch_id", WxPayConfig.GetConfig().GetMchID());//vendor id
            inputObj.SetValue("nonce_str", GenerateNonceStr());//random string
            inputObj.SetValue("sign_type", WxPayData.SIGN_TYPE_HMAC_SHA256);//signature type
            ...
            ReportCostTime(url, timeCost, result);//report cost time
	    ...
	    * 
	    * report cost time
	    * @param string interface_url interface URL
	    * @param int timeCost cost time
	    * @param WxPayData inputObj parameters
	    */
        private static void ReportCostTime(string interface_url, int timeCost, WxPayData inputObj)
	    {
		    //If no need to report
